...
- **high**: The 5 neighbors form a tight, coherent cluster with a clear unifying theme (e.g., all use network methods for cancer drug targets). The target article strongly aligns with this theme—sharing the core research question, methodology, or application domain with **at least 4 of 5 neighbors**. Tag overlap with the cluster's dominant tags is substantial.

- **medium**: The neighbors show moderate coherence—perhaps 3-4 neighbors share a theme while 1-2 are more distant. The target article partially aligns with the dominant theme—sharing methodological approaches or application domains with **2-3 neighbors**, but with notable differences (e.g., different disease, complementary method, or tangential research angle).

- **low**: EITHER he neighbors lack coherence (each addresses a different topic/method), OR the target article has minimal alignment with an otherwise coherent cluster (shares only superficial connections like broad tags or journal, but not specific research focus). Target aligns with **fewer than 2 neighbors** in meaningful ways.

# Evaluation Strategy

1. **Identify cluster consistency**: Analyze the 5 neighbor articles to determine if they form a coherent thematic/methodological cluster. Identify common topics, methodologies, and research themes
3. **Compare target to cluster**: Assess how well the target article aligns with these identified themes
4. **Weight the evidence**: Consider title relevance, abstract content, and methodological similarity
5. **Assign level**: Based on the overall alignment, assign high/medium/low relevance
